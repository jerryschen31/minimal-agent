...
type ContextWindow interface {
	...
	Compact(ContextState, summaryMsg ChatMessage) bool
	Clear()
}

// Compact(stateMessageIDs, summaryMsg, state.Gen)
...
func (w *OffsetWindow) Compact(state ContextState, summaryMsg ChatMessage) bool {
	...
	stateMessageIDs := make(map[string]bool, len(state.Messages))
	for _, m := range state.Messages {
		stateMessageIDs[m.ID] = true
	}
...
func runLoop(ctx context.Context, provider Provider, systemPrompt string) {
	...
	for {
		...
		if strings.HasPrefix(line, "/summary") {
			summary, err := summarizeChatHistory(ctx, provider, chatHistory.GetMessages())
			if err != nil {
				fmt.Fprintln(os.Stderr, "error:", err)
				continue
			}
			fmt.Println("Chat summary:", summary)
